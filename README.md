...
默认优先使用火山引擎 TTS，失败自动回退到 macOS `say`。
...
| `VOICE` | `Samantha` | 回退 `say` 时声音 |
| `TTS_PROVIDER` | `doubao` | `doubao` 或 `say` |
| `VOLC_TTS_APPID` | 空 | 火山 TTS AppID |
| `VOLC_TTS_TOKEN` | 空 | 火山 Access Token |
